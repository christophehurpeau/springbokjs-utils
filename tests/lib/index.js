...
test("some should forEach elements of an Array until the callback returns true", function () {
    var results = [];
    var result = utils.some([1, 2, 3], function (v, i) {
        results.push(v);return v === 2;
    }, 5);
    expect(result, true);
...
test("some should forEach elements of an Object until the callback returns true", function () {
    var results = {};
    var result = utils.some({ a: 1, b: 2, c: 3 }, function (v, key) {
        results[key] = v;return v === 2;
    }, 5);
    expect(result, true);
    assert.deepEqual(results, { a: 1, b: 2 });
});

test("map to array should transform a map into an array", function () {
    var map = new Map();
    map.set("a", 1);
    map.set("ab", 2);
    var result = utils.mapToArray(map, function (v, k) {
        return k + ":" + v;
    });
...
test("map to array should transform a object into an array", function () {
    var object = { a: 1, ab: 2 };
    var result = utils.mapToArray(object, function (v, k) {
        return k + ":" + v;
    });
    assert.isArray(result);
    assert.deepEqual(result, ["a:1", "ab:2"]);
...
test("map to array should transform an array into another array", function () {
    var object = [1, 2];
    var result = utils.mapToArray(object, function (v, k) {
        return k + ":" + v;
    });
...
test("defineGetter should work", function () {
    var o = {};
    var res = utils.defineGetter(o, "a", function () {
        return 1;
    });
    expect(res, o);
    expect(res.a, 1);
...
test("defineSetter should work", function () {
    var o = {};
    var value;
    var res = utils.defineSetter(o, "a", function (v) {
        return value = v * 2;
    });
    expect(res, o);
    res.a = 2;
